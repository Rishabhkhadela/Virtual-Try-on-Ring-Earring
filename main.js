...
function processResults(results) {
  ...
  // -- Manual position offset sliders (calibration only) ------------------
  // Applied in the scene-space basis we just built. Default sliders are 0,
  // so this block is a no-op for normal use; lives here to keep the tweak
  // engine usable for one-off calibration without touching code.
  ...
  if (offsetX || offsetY || offsetZ) {
    const handednessSignX = isReportedRightHand ? 1 : -1;
    targetPos
      .addScaledVector(_tmpDirX, segLengthWorld * offsetX * handednessSignX)
      .addScaledVector(_tmpDirD, segLengthWorld * offsetY)
      .addScaledVector(_tmpDirN, fingerRadiusScene * offsetZ);
  }
